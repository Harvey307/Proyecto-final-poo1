...
public:
	...
	void busqueda_nomape(string nomp)
	{
		//AGREGADOOOO PARA LA BUSQUEDA DE NOMBRE COINCIDENCIAS
		Cliente busqueda_nomape(string nomp);
		{
			ClienteVector clientevector1;
			for (int i = 0; i < clientevector1.rows(); i++)
			{
				if (clientevector1.get(i).getnomape() == nomp)
				{
					 cout <<"                                           "<<"------------------------------" << endl;
					 cout <<"                                           "<<"Nombre:" << clientevector1.get(i).getnomape() << " " << "codigo : " << clientevector1.get(i).getcodigo() << endl;
	...
	//otra busqueda opcion 2
	/*void busqueda_nomape(string nomp)
	{
		ClienteVector clientevector1;
		for (int i = 0; i < clientevector1.rows(); i++) {
			if (clientevector1.get(i).getnomape() == nomp) {
				cout << clientevector1.get(i).getnomape() << " " << clientevector1.get(i).getcodigo() << endl;
			}
		}
